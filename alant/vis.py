...
from data.rotation_conversions import axis_angle_to_quaternion, quaternion_multiply, quaternion_apply
...
def skeleton_render(
    ...
    if render:
        ...
        gifname = os.path.join(out, f"{pathparts[-1][:-4]}" + str(name) +".motion.html")
        writervideo = animation.HTMLWriter(fps=20)
        anim.save(gifname, writer=writervideo)
    plt.close()
