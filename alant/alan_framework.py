...
from alan_dataset import MotionDataset
from dataset.preprocess import increment_path
from model.adan import Adan
from alan_model import GaussianDiffusion
...
class MotionModel:
    def __init__(
        ...
        self.repr_dim = repr_dim = pos_dim + rot_dim
    ...
    def render_sample(          # This is inference
            self, data_tuple, label, render_dir, render_count=-1
    ):
        self.eval()
...
def train(opt):
    ...
    opt.batch_size = 64     # !!! change back to 512
...
def test(opt):
    ...
    label = '_test_1121'
    render_dir = os.path.join(opt.render_dir, opt.exp_name + label)

    model.render_sample(data_tuple, label, render_dir, render_count)


if __name__ == "__main__":
    opt = parse_train_opt()
    opt.checkpoint = "./train-1550.pt"
    train(opt)
    test(opt)
